...
def get_arg_parser():
    ...
    parser.add_argument('--code-dir', dest='code_dir', action='store',
                        required=False,
                        help='A folder to put the source of the packages')

    parser.add_argument('--usr-dir', dest='usr_dir', action='store',
                        required=False,
                        help='A folder to install the packages')

    parser.add_argument('--packages-file', dest='packages_file',
                        action='store', help='File to read the packages from',
    ...
    parser_install.add_argument('packages', nargs='*', default=None,
                                help='A list of packages to install')
    parser_install.add_argument('--reinstall', dest='reinstall',
                                action='store_true',
                                help='Should the packages be reinstalled')
    parser_install.add_argument('--group', action='store', metavar='name',
                                default=None,
                                help='Should the packages be reinstalled')
    ...
    parser_fetch.add_argument('packages', nargs='*',
                              default=None, help='A list of packages to fetch')
    parser_fetch.add_argument('--focre--clear', dest='force_clear',
                              action='store_true', default=False,
                              help='Will delete any\
                              folders that stay on its way')
    parser_fetch.add_argument('--group', action='store', metavar='name',
                              default=None, help='If given, every\
    ...
    parser_build.add_argument('packages', nargs='*', default=None,
                              help='A list of packages to fetch')
    parser_build.add_argument('--group', action='store', metavar='name',
                              default=None, help='If given, every package\
                              from this group will be build')
    parser_build.add_argument('--no-install', dest='noinstall',
                              action='store_true', default=False,
...
def setup_config_files(args, opt):
    ...
    if not os.path.isdir(os.path.join(code_manager.CONFDIR, "install_scripts")):
        copytree(os.path.join(code_manager.CMDIR, "install_scripts"),
