...
class BasicInstaller(ConfigurationAware):
    ...
    def execute(self, name):
        raise NotImplementedError(
            'All installers need to implement the execute method.',
        )

    @abstractmethod
    def update(self, name):
        raise NotImplementedError(
            'All installers need to implement the update method.',
        )
...
class Installation(ConfigurationAware):
    ...
    def load_installer(self):
        ...
        import_modules_from_folder(
            self.installers_dir, 'code_manager.installers', self._add_installer,
        )
    ...
    def _add_installer(self, installer, file):
        ...
        if InstallerClass.name in self.installers.keys():
            debug_red(
                'Installer with the name \'%s\' already exists',
                InstallerClass.name,
            )
    ...
    def run_installer(self, name, installer, node):
        ...
        if installer not in self.installers.keys():
            logging.critical(
                'There is no installer with the name %s', installer,
            )
...
but it is not in the package node of %s.', attr, installer_obj.name, name,
        ...
        if result is None:
            logging.critical(
                'The installer [%s] failed to execute properly', installer_obj.name,
            )
